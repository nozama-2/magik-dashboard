      ...
      <div className="flex-1 space-y-4 p-8 pt-6">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          <Link to="/CreateQuiz">
            ...
            >
              <PlusIcon className="mr-2 h-20 w-20" /> Create a Quiz
            </Button>
          </Link>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-7">
          {Object.keys(quizzes).map((qk) => (
            <RecentQuizButton
              name={qk}
              // description="Objective: Teach students the basics of single line
              // equations."
              description={quizzes[qk].objective}
              noOfQuestions={quizzes[qk].questions.length}
              image={quizzes[qk].image}
            />
          ))}
        </div>
      </div>
